    ...
    */
    static validate(cliente) {
        ...
        if (!cliente.correo.trim())
            throw new EmptyInfoError("El correo no puede estar vacio");
        if (!validate_email(cliente.correo))
            throw new EmailDenied("El correo no es valido");
        if (!cliente.whatsapp.trim())
            throw new EmptyInfoError("El WhatsApp no puede estar vacio");
    }
}
