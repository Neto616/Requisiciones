...
class UsuarioRepository extends ClaseRepo {
    db;
    constructor(db) {
        super();
        this.db = db;
    ...
    async getID(data) {
        ...
        if (!busqueda.length)
            throw new ErrorUserNotExist("El usuario que busca no existe");
    ...
    async getInfo(id) {
        const query = `
            ...
            where id = $1;
        `;
        const user_info = await this.db.query(query, [id]);
        if (!user_info.length)
            return [];
        const array_user_info = user_info.map(e => ({
            id: e.id,
     ...
     */
    async crear(data, other_data = { tipo: TIPO_USER, estatus: ESTATUS_CLIENTE }) {
        if ((await this.isExist(data)))
            throw new ErrorUserExists("El usuario ya existe");
        const query = `
    ...
    async eliminar(id) {
        if (!(await this.isExistById(id)))
            throw new ErrorUserNotExist("El usuario a borrar no existe");
        const query = `
            UPDATE usuarios
            SET estatus = $1
            WHERE id = $2
